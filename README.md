...
- **例句** — 每個單字都附一句英文例句（粗體標出目標單字）與中文翻譯，翻面時顯示、可朗讀。
...
data/examples.json         # 例句資料（id → {en, zh}），已生成並提交
scripts/extract-pdf.mjs    # PDF → ielts-raw.txt（一次性，需 pdfjs-dist）
scripts/build-deck.mjs     # ielts-raw.txt + examples.json → src/deck.json（dev/build 前自動執行）
scripts/generate-examples.mjs # （選用）用 Claude 批次生成例句 → data/examples.json
...
# （選用）為尚未有例句的單字補生成例句，需 API key：
npm i -D @anthropic-ai/sdk
ANTHROPIC_API_KEY=sk-ant-... npm run gen:examples              # 只補缺漏，可增量執行
```
